...
export async function getAllTypeScriptFilesFromMultipleDirectoriesAsync(dirPathArr: string[]): Promise<string[]> {
    return Promise.all(dirPathArr.map(async (dirPath) => getAllTypeScriptFilesAsync(dirPath))).then((data) => data.flat());
}

export function findAllEnumsInFile(fileContent: string, filePath: string): Array<IEnumInfo> {
    const res: IEnumInfo[] = [];

    findBody({ startWord: 'enum', fileContent }).forEach((interfaceBody) => {
...
export function findAllInterfacesInFile(fileContent: string, filePath: string): Array<IInterfaceInfo> {
...
export function findAllRoutesInFile(fileContent: string, filePath: string): Array<IRouteInfo> {
